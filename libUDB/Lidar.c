...
void rxLidar_on_PWM(lidar_callback_funcptr callback)  // service the Lidar altimeter on PWM
{
	...
	long altitude;  // in mm
        ...
        accum.WW = udb_pwIn[LIDAR_INPUT_CHANNEL]  - (lidar0 << 1);// Offset correction for the LidarLite (mm);
        ...
        if (accum.WW < 30000) {
		   ...
		   altitude = (long)((accum._.W1 << 1) );
                    //raw uncorrected altitude :altitude = ((long)udb_pwIn[LIDAR_INPUT_CHANNEL] >> 1)
                }
