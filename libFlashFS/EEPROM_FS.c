...
// the FAT table to reference up to 341*512 = ~174kB of space.  Therefore, more 
// FAT sectors are needed if creating an MSD volume bigger than this.
#define MDD_EEPROM_FLASH_NUM_RESERVED_SECTORS 1
#define MDD_EEPROM_FLASH_NUM_VBR_SECTORS 1
#define MDD_EEPROM_FLASH_NUM_FAT_SECTORS 1
#define MDD_EEPROM_FLASH_NUM_ROOT_DIRECTORY_SECTORS ((MDD_EEPROM_FLASH_MAX_NUM_FILES_IN_ROOT+15)/16) //+15 because the compiler truncates

//#define MDD_EEPROM_FLASH_PARTITION_SIZE (DWORD)(MDD_EEPROM_FLASH_TOTAL_DISK_SIZE - 1)  //-1 is to exclude the sector used for the MBR 
#define MDD_EEPROM_FLASH_PARTITION_SIZE (DWORD)(MDD_EEPROM_FLASH_TOTAL_DISK_SIZE)
...
MDD_EEPROM_FLASH_NUM_RESERVED_SECTORS, 0x00,         // Reserved sector count (usually 1 for FAT12 or FAT16)
0x01,                                               // Number of FATs 
MDD_EEPROM_FLASH_MAX_NUM_FILES_IN_ROOT, 0x00,        // Max number of root directory entries
//0x00, 0x00,                                       // Total sectors (0x0000 means: use the 4 byte field at offset 0x20 instead)
(BYTE)MDD_EEPROM_FLASH_PARTITION_SIZE, (BYTE)(MDD_EEPROM_FLASH_PARTITION_SIZE >> 8),    // Total sectors (0x0000 means: use the 4 byte field at offset 0x20 instead)
0xF8,                                               // Media Descriptor
MDD_EEPROM_FLASH_NUM_FAT_SECTORS, 0x00,              // Sectors per FAT
0x3F, 0x00,                                         // Sectors per track
0xFF, 0x00,                                         // Number of heads
//0x01, 0x00, 0x00, 0x00,                           // Hidden sectors
0x00, 0x00, 0x00, 0x00,                             // Hidden sectors
//(BYTE)MDD_EEPROM_FLASH_PARTITION_SIZE, (BYTE)(MDD_EEPROM_FLASH_PARTITION_SIZE >> 8), (BYTE)(MDD_EEPROM_FLASH_PARTITION_SIZE >> 16), (BYTE)(MDD_EEPROM_FLASH_PARTITION_SIZE >> 24), // Total sectors (when WORD value at offset 20 is 0x0000)
...
//	unsigned char buf[520];
	...
	WriteSector(1, buf);
	memset(buf, '\0', 512);
	for (i = 2; i < (MDD_EEPROM_FLASH_NUM_FAT_SECTORS + 1); i++) {
		WriteSector(i, buf);
	}
	...
	WriteSector(MDD_EEPROM_FLASH_NUM_FAT_SECTORS+1, buf);
	memset(buf, '\0', 512);
	for (i = (MDD_EEPROM_FLASH_NUM_FAT_SECTORS+1+1); i < ((MDD_EEPROM_FLASH_NUM_FAT_SECTORS+1) + MDD_EEPROM_FLASH_NUM_ROOT_DIRECTORY_SECTORS); i++) {
		WriteSector(i, buf);
	}
	printf("EEPROM_FormatFS() complete\r\n");
}
